...
Codex uses these shared repository settings:
- `AGENTS.md` — durable project instructions and development workflows
- `.codex/hooks.json` — PreToolUse hook registration; commands resolve from
  the Git root so the configuration works in every clone and subdirectory
- `.claude/hooks/` — agent-neutral hook scripts shared with Claude Code:
  `validate-commit-msg.sh` (Conventional Commits), `block-main-push.sh`
  (no direct pushes to main; release tags allowed), and `validate-pr.sh`
  (PR title format + required template sections)

Project-local Codex hooks run only for trusted repositories. Review or manage
them with `/hooks`. Keep personal Codex defaults in `~/.codex/config.toml`;
add `.codex/config.toml` only when a setting must be shared by this project.
Except for this Codex-specific section, keep shared project guidance aligned
with `CLAUDE.md`.
...
## Performance Tuning
- Default chunk size: 256KB, and there is rarely a reason to change it — per-character work is
  constant, so throughput is flat across chunk sizes and correctness never depends on where
  chunks are cut (see @docs/PERFORMANCE.md)
- Chunk size can still be customized via `--chunk-kb` if a chunk-count/parallelism tradeoff is
  needed (see docs/PERFORMANCE.md's "Chunk Size" section)
- Thread count can be specified via `--threads`; parallel setup cost usually outweighs the gain
  below ~1MB of input, which is what the adaptive default encodes
  - Example: `sakurs process -i large.txt --threads 8`
...
4. Do NOT add AI attribution footers (`🤖 Generated with ...` /
   `Co-Authored-By: Codex ...`) to commits or PR bodies.
...
3. **Commit Process**:
   ...
   - Create commit with conventional commit format (validated by
     `.claude/hooks/validate-commit-msg.sh` through `.codex/hooks.json`)
