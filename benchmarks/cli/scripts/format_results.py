...
def generate_latex_tables(results: Dict[str, List[Dict]], output_dir: Path):
    ...
    if results['accuracy']:
        ...
        latex = df.to_latex(index=False, caption="Segmentation Accuracy Results", 
                           label="tab:accuracy")
    ...
    if results['performance']:
        ...
        latex = df.to_latex(index=False, caption="Performance Benchmark Results",
...
def generate_plots(results: Dict[str, List[Dict]], output_dir: Path):
    ...
    if results['accuracy']:
        ...
        # F1 score comparison
        plt.figure()
        ax = df.plot(x='Dataset', y='F1', kind='bar', legend=False)
    ...
    if results['performance']:
        ...
        plt.figure()
        ax = df.plot(x='Dataset', y='Throughput (MB/s)', kind='bar', legend=False)
...
@click.option('--output-dir', '-o', type=click.Path(),
              help='Output directory for formatted results')
