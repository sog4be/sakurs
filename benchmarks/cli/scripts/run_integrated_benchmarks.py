        ...
        for tool in config["tools"]:
            if tool == "sakurs":
                ...
                for threads in [1, 2, 4, 8]:
                    print(f"  Testing {tool} with {threads} threads on {config['language']} data...")
...
def run_accuracy_benchmarks(data_dir: Path, results: ExperimentResults):
    ...
    configs = [
        ...
        {
            "language": "JA",
            "test_file": data_dir / "ud_japanese_bccwj" / "test.txt",
            "gold_file": data_dir / "ud_japanese_bccwj" / "test_gold.txt",
            "tools": ["sakurs", "ja_sentence_segmenter"]
        },
        # English tests
        {
