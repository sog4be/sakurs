...
def add_confidence_intervals_to_metrics(
    metrics: dict[str, float], n_samples: int | None = None, timing_data: list[float] | None = None
    ...
    for metric_name in proportion_metrics:
        if metric_name in metrics:
            ...
            if n_samples:
                ...
                ci = ConfidenceInterval(value, max(0, value - 1.96 * se), min(1, value + 1.96 * se))
...
def paired_t_test(
    sample1: list[float], sample2: list[float], alternative: str = "two-sided"
) -> tuple[float, float]:
...
def calculate_effect_size(sample1: list[float], sample2: list[float]) -> dict[str, float]:
    ...
    return {"cohens_d": float(cohens_d), "hedges_g": float(hedges_g)}
