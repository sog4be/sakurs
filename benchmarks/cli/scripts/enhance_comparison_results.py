...
from comparison_tests import approximate_randomization_test
from statistical_analysis import calculate_effect_size, paired_t_test, wilcoxon_signed_rank_test
...
def enhance_comparison_analysis(analysis_json_path: Path) -> dict[str, Any]:
    ...
    if "sakurs" in analysis and "comparison" in analysis:
        ...
        baseline_time = analysis.get("punkt", analysis.get("ja_sentence_segmenter", {})).get(
            "mean_time"
        )
        ...
        if baseline_time:
            ...
            baseline_std = analysis.get("punkt", analysis.get("ja_sentence_segmenter", {})).get(
                "stddev", baseline_time * 0.05
            )
...
def generate_statistical_report(comparison_dir: Path, output_path: Path | None = None) -> str:
    ...
    report_lines = ["# Statistical Comparison Report", "", "## Executive Summary", ""]
    ...
    for analysis_file in analysis_files:
        ...
        if "statistical_tests" in enhanced.get("comparison", {}):
            ...
            all_comparisons.append({"dataset": dataset, "p_value": summary["p_value"]})
...
@click.option("--output", "-o", type=click.Path(), help="Output path for statistical report")
def main(comparison_dir, output):
    ...
    for analysis_file in analysis_files:
        ...
        enhanced_path = analysis_file.with_name(analysis_file.stem + "_enhanced.json")
