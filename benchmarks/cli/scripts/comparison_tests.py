...
from statistical_analysis import calculate_effect_size, format_statistical_result
...
def mcnemar_test(
    ...
    a = sum(1 for s1, s2 in zip(system1_correct, system2_correct) if s1 and s2)
    ...
    d = sum(1 for s1, s2 in zip(system1_correct, system2_correct) if not s1 and not s2)
...
def compare_accuracy_results(
    ...
    for metric in ['precision', 'recall', 'f1', 'pk', 'window_diff']:
        if metric in metrics1 and metric in metrics2:
            ...
            val1 = metrics1[metric]['estimate'] if isinstance(metrics1[metric], dict) else metrics1[metric]
            val2 = metrics2[metric]['estimate'] if isinstance(metrics2[metric], dict) else metrics2[metric]
