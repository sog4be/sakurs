...
impl EnclosureStateTracker {
    ...
    pub fn update(&mut self, enclosure_type: EnclosureType, type_id: usize, is_opening: bool, position: usize) {
        if is_opening {
            ...
            *self.depth_map.entry(type_id).or_insert(0) += 1;
            ...
            if let Some(pos) = self.open_enclosures.iter().rposition(|(t, _)| *t == enclosure_type) {
                self.open_enclosures.remove(pos);
                self.unclosed_positions.retain(|&p| p != self.open_enclosures.get(pos).map(|(_, p)| *p).unwrap_or(0));
            } else {
    ...
    pub fn merge_with_next(&self, next: &Self) -> Self {
        ...
        merged.open_enclosures.extend(next.open_enclosures.iter().cloned());
        ...
        if next.unopened_closures.len() > 0 && self.open_enclosures.len() > 0 {
...
impl CrossChunkValidator {
        ...
        if near_end {
            if let Some(next) = next_state {
                if state.abbreviation.is_cross_chunk_abbr(&next.abbreviation) {
                    return ValidationResult::Invalid("Cross-chunk abbreviation detected".to_string());
                }
            }
        }
    ...
    fn validate_enclosure_balance(&self, boundary: &BoundaryCandidate, _state: &PartialState) -> bool {
...
mod tests {
    ...
    fn test_cross_chunk_validator() {
        ...
        let result = validator.validate_chunk_boundary(&boundary, &state, None, &crate::domain::language::MockLanguageRules::english());
